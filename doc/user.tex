...
Introduction. There is no packaged \texttt{dash-lineplot.exe} or
\texttt{startPlotTool.bat} in the current version, no subplot option, and
no x-axis slider (replaced by the X/Y range boxes beside each graph). For
current usage instructions, see \texttt{docs/userguide.md} in the
repository root.
\end{quotation}

\label{chap:userLevelDescription}
\section{Starting the Dash Line Plot Graphing Utility (historical)}


The application was distributed, in the version this chapter describes,
...
Only the Python script exists today; run it directly with
\texttt{python dash-lineplot.py --configfile dash-config.xlsx} as described
in \texttt{docs/userguide.md}.
...
\clearpage
\subsection{Slider Usage (removed)}

\textit{The slider described in this subsection no longer exists. It was
replaced by the X range and Y range boxes beside each graph, which take
effect immediately on Apply rather than requiring a click on the tab to
redraw. See ``Setting the axis ranges by typing them'' in
\texttt{docs/userguide.md}.}
...
\begin{description}
  \item [Click Data:] The user can click on any trace to record the clicked point in the \texttt{Click Data} box below the relevant graph. When a second data point is clicked, the range in x and y are reflected in the display text box. This functionality is available for individual Plotly figures (subplots have since been removed; see the notice above). The click functionality can be used in conjunction with the standard Plotly zoom functionality.
