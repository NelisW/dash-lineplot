...
\textit{As of this writing the script no longer opens a desktop window at
all (see the notice in the Introduction); it serves its page directly to
the system browser. The paragraph below describing a browser widget inside
a window is historical.}
...
\begin{itemize}
  ...
  \item \ac{JSON} files holding either one record array or an object of named groups, one group per sample rate.
\end{itemize}

Matlab format files are no longer supported: the reader that loaded data
from \texttt{DATA}/\texttt{NAM}/\texttt{TIME} was removed along with the
\texttt{scipy} dependency it needed. See \texttt{docs/userguide.md} for the
current, authoritative list of supported file types.
